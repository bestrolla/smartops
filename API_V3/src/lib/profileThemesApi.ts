...
class ProfileThemesApi {
  ...
  async getThemes(params?: {
    ...
    const response = await apiClient.get(`/profiles/themes?${queryParams.toString()}`);
  ...
  async getThemesByCategory(category: string): Promise<ProfileThemesResponse> {
    const response = await apiClient.get(`/profiles/themes/category/${category}`);
    return response.data;
  }
  ...
  async searchThemes(query: string): Promise<ProfileThemesResponse> {
    const response = await apiClient.get(`/profiles/themes/search?q=${encodeURIComponent(query)}`);
    return response.data;
  }
  ...
  async getThemeById(themeId: string): Promise<{ success: boolean; data: ProfileTheme }> {
    const response = await apiClient.get(`/profiles/themes/${themeId}`);
    return response.data;
  }
  ...
  async getPopularThemes(limit: number = 10): Promise<ProfileThemesResponse> {
    const response = await apiClient.get(`/profiles/themes/popular?limit=${limit}`);
    return response.data;
  }
  ...
  async getThemeStats(): Promise<{
    ...
    const response = await apiClient.get('/profiles/themes/stats');
    return response.data;
  }
  ...
  async getTenantThemes(tenantId: string, params?: {
    ...
    const response = await apiClient.get(`/profiles/${tenantId}/themes?${queryParams.toString()}`);
    return response.data;
  }
  ...
  async applyThemeToProfile(tenantId: string, themeId: string): Promise<ApplyThemeResponse> {
    const response = await apiClient.post(`/profiles/${tenantId}/themes/apply`, {
      theme_id: themeId
    });
    return response.data;
  }
  ...
  async incrementThemeUsage(themeId: string): Promise<{
    ...
    const response = await apiClient.post(`/profiles/themes/${themeId}/usage`);
    return response.data;
  }
  ...
  async addThemeReview(themeId: string, review: {
    ...
    const response = await apiClient.post(`/profiles/themes/${themeId}/review`, review);
    return response.data;
  }
}
